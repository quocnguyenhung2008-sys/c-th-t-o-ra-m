"""
gui_app.py — Giao diện đồ họa cho công cụ Phân loại Tài liệu THPT
Yêu cầu: pip install customtkinter
Chạy: python gui_app.py
"""
from __future__ import annotations

import csv
import json
import threading
import time
from pathlib import Path
from tkinter import filedialog, messagebox
from typing import Any

import customtkinter as ctk

# ── Thiết lập giao diện toàn cục ─────────────────────────────────────────────
ctk.set_appearance_mode("System")          # Tự động theo Dark/Light của hệ điều hành
ctk.set_default_color_theme("blue")


# ═════════════════════════════════════════════════════════════════════════════
# PALETTE & TYPOGRAPHY
# Ngôn ngữ thiết kế: "Phòng lab tối giản" — nền xám lạnh, accent xanh điện,
# font monospace cho dữ liệu, không bo góc quá mức.
# ═════════════════════════════════════════════════════════════════════════════
PAL = {
    "accent":       "#3B82F6",   # blue-500
    "accent_dark":  "#2563EB",   # blue-600
    "success":      "#22C55E",   # green-500
    "warning":      "#F59E0B",   # amber-500
    "danger":       "#EF4444",   # red-500
    "card_light":   "#F1F5F9",
    "card_dark":    "#1E293B",
    "text_muted":   "#64748B",
}

FONT_DISPLAY = ("SF Pro Display", 22, "bold")
FONT_LABEL   = ("SF Pro Text",   13)
FONT_MONO    = ("JetBrains Mono", 12)
FONT_SMALL   = ("SF Pro Text",   11)


# ═════════════════════════════════════════════════════════════════════════════
# CORE LOGIC — Phân tích CSV + Aliases
# ═════════════════════════════════════════════════════════════════════════════

def load_aliases(path: Path) -> dict[str, list[str]]:
    """Nạp file aliases.json → dict {môn: [alias1, alias2, ...]}"""
    with path.open("r", encoding="utf-8") as f:
        raw = json.load(f)
    result: dict[str, list[str]] = {}
    for subject, values in raw.items():
        if isinstance(values, list):
            result[subject] = [str(v).strip().lower() for v in values if str(v).strip()]
        elif isinstance(values, dict):
            result[subject] = [str(k).strip().lower() for k in values if str(k).strip()]
    return result


def save_aliases(path: Path, data: dict[str, list[str]]) -> None:
    """Ghi dict aliases về file JSON (giữ nguyên định dạng list)."""
    with path.open("w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)


def load_report(path: Path) -> list[dict[str, str]]:
    """Đọc classification_report.csv → list[dict]"""
    rows: list[dict[str, str]] = []
    with path.open("r", encoding="utf-8-sig", newline="") as f:
        reader = csv.DictReader(f)
        for row in reader:
            rows.append(dict(row))
    return rows


def analyse_report(
    rows: list[dict[str, str]],
    aliases: dict[str, list[str]],
    progress_cb=None,
) -> tuple[list[dict], float, int]:
    """
    Phân tích báo cáo CSV và đề xuất nhãn mới dựa trên aliases.
    Trả về: (danh sách thay đổi, accuracy%, số file được sửa)
    """
    changes: list[dict] = []
    total = len(rows)
    correct = 0

    # Xây dựng bảng tra cứu alias → môn
    alias_map: dict[str, str] = {}
    for subject, alias_list in aliases.items():
        for alias in alias_list:
            alias_map[alias.lower()] = subject

    for idx, row in enumerate(rows):
        if progress_cb:
            progress_cb((idx + 1) / total)

        # Lấy các cột có thể có trong CSV (linh hoạt tên cột)
        filename   = row.get("filename") or row.get("file") or row.get("Filename") or ""
        old_label  = row.get("target_label") or row.get("label") or row.get("Label") or ""
        status     = row.get("status") or row.get("Status") or ""

        stem = Path(filename).stem.lower()

        # Thử khớp từng alias trong tên file
        matched_subject: str | None = None
        for alias, subject in sorted(alias_map.items(), key=lambda x: -len(x[0])):
            if alias in stem:
                matched_subject = subject
                break

        new_label = matched_subject or old_label

        if new_label == old_label:
            correct += 1
        else:
            changes.append({
                "filename":  filename,
                "old_label": old_label,
                "new_label": new_label,
                "status":    status,
            })

        # Thêm delay nhỏ để thanh tiến trình hiển thị (simulate xử lý)
        time.sleep(0.003)

    accuracy = (correct / total * 100) if total else 0.0
    return changes, accuracy, len(changes)


# ═════════════════════════════════════════════════════════════════════════════
# COMPONENT: MetricCard — Thẻ chỉ số đơn
# ═════════════════════════════════════════════════════════════════════════════

class MetricCard(ctk.CTkFrame):
    def __init__(self, master, title: str, value: str = "—", accent: str = PAL["accent"], **kw):
        super().__init__(master, corner_radius=12, **kw)
        self._accent = accent

        self._title_lbl = ctk.CTkLabel(
            self, text=title.upper(),
            font=("SF Pro Text", 10, "bold"),
            text_color=PAL["text_muted"],
        )
        self._title_lbl.pack(anchor="w", padx=16, pady=(14, 0))

        self._value_lbl = ctk.CTkLabel(
            self, text=value,
            font=("SF Pro Display", 28, "bold"),
            text_color=accent,
        )
        self._value_lbl.pack(anchor="w", padx=16, pady=(2, 14))

    def set_value(self, value: str) -> None:
        self._value_lbl.configure(text=value)


# ═════════════════════════════════════════════════════════════════════════════
# COMPONENT: DataTable — Bảng kết quả (Treeview thuần tkinter bọc trong CTk)
# ═════════════════════════════════════════════════════════════════════════════

class DataTable(ctk.CTkFrame):
    """Bảng hiển thị danh sách thay đổi nhãn."""

    COLUMNS = (
        ("filename",  "Tên file",    320),
        ("old_label", "Nhãn cũ",     150),
        ("new_label", "Nhãn mới",    150),
        ("status",    "Trạng thái",  110),
    )

    def __init__(self, master, **kw):
        super().__init__(master, corner_radius=10, **kw)

        import tkinter.ttk as ttk
        import tkinter as tk

        # Style cho Treeview (không inherit màu CTk nên phải set thủ công)
        style = ttk.Style()
        style.theme_use("default")
        style.configure(
            "DocSort.Treeview",
            background="#1E293B",
            foreground="#E2E8F0",
            fieldbackground="#1E293B",
            borderwidth=0,
            rowheight=30,
            font=("JetBrains Mono", 11),
        )
        style.configure(
            "DocSort.Treeview.Heading",
            background="#0F172A",
            foreground="#94A3B8",
            font=("SF Pro Text", 11, "bold"),
            borderwidth=0,
        )
        style.map("DocSort.Treeview", background=[("selected", "#2563EB")])

        cols = [c[0] for c in self.COLUMNS]
        self._tree = ttk.Treeview(
            self, columns=cols, show="headings",
            style="DocSort.Treeview", selectmode="browse",
        )
        for col_id, col_title, col_width in self.COLUMNS:
            self._tree.heading(col_id, text=col_title)
            self._tree.column(col_id, width=col_width, minwidth=80, anchor="w")

        # Scrollbar dọc
        vsb = ctk.CTkScrollbar(self, command=self._tree.yview)
        self._tree.configure(yscrollcommand=vsb.set)

        self._tree.pack(side="left", fill="both", expand=True, padx=(0, 0))
        vsb.pack(side="right", fill="y")

    def load(self, rows: list[dict]) -> None:
        """Xóa dữ liệu cũ và nạp list mới."""
        for item in self._tree.get_children():
            self._tree.delete(item)
        for row in rows:
            tag = "changed" if row.get("new_label") != row.get("old_label") else ""
            self._tree.insert("", "end", values=(
                row.get("filename", ""),
                row.get("old_label", ""),
                row.get("new_label", ""),
                row.get("status", ""),
            ), tags=(tag,))
        self._tree.tag_configure("changed", foreground="#FCD34D")

    def get_all_rows(self) -> list[tuple]:
        return [self._tree.item(i)["values"] for i in self._tree.get_children()]


# ═════════════════════════════════════════════════════════════════════════════
# TAB 1: PHÂN TÍCH & TỐI ƯU
# ═════════════════════════════════════════════════════════════════════════════

class AnalysisTab(ctk.CTkFrame):
    def __init__(self, master, app: "App", **kw):
        super().__init__(master, fg_color="transparent", **kw)
        self._app = app
        self._csv_path: Path | None = None
        self._result_rows: list[dict] = []
        self._build()

    def _build(self) -> None:
        # ── Tiêu đề ──────────────────────────────────────────────────────────
        header = ctk.CTkFrame(self, fg_color="transparent")
        header.pack(fill="x", padx=24, pady=(20, 0))

        ctk.CTkLabel(
            header, text="Phân tích & Tối ưu",
            font=("SF Pro Display", 20, "bold"),
        ).pack(side="left")

        # ── Vùng chọn file ───────────────────────────────────────────────────
        file_frame = ctk.CTkFrame(self, corner_radius=12)
        file_frame.pack(fill="x", padx=24, pady=(16, 0))

        ctk.CTkLabel(
            file_frame, text="BÁO CÁO CSV",
            font=("SF Pro Text", 10, "bold"), text_color=PAL["text_muted"],
        ).grid(row=0, column=0, sticky="w", padx=16, pady=(14, 0))

        self._csv_entry = ctk.CTkEntry(
            file_frame, placeholder_text="Chọn file classification_report.csv...",
            font=FONT_MONO, height=36, corner_radius=8,
        )
        self._csv_entry.grid(row=1, column=0, sticky="ew", padx=16, pady=(4, 14))

        ctk.CTkButton(
            file_frame, text="Browse", width=90, height=36,
            corner_radius=8, fg_color=PAL["accent"], hover_color=PAL["accent_dark"],
            command=self._browse_csv,
        ).grid(row=1, column=1, padx=(0, 16), pady=(4, 14))

        file_frame.columnconfigure(0, weight=1)

        # ── Nút điều khiển ───────────────────────────────────────────────────
        ctrl = ctk.CTkFrame(self, fg_color="transparent")
        ctrl.pack(fill="x", padx=24, pady=(12, 0))

        self._run_btn = ctk.CTkButton(
            ctrl, text="▶  Bắt đầu phân tích",
            height=40, corner_radius=8,
            fg_color=PAL["accent"], hover_color=PAL["accent_dark"],
            font=("SF Pro Text", 13, "bold"),
            command=self._run_analysis,
        )
        self._run_btn.pack(side="left")

        self._export_btn = ctk.CTkButton(
            ctrl, text="↓  Xuất kết quả (.csv)",
            height=40, corner_radius=8, width=180,
            fg_color="transparent", border_width=1,
            border_color=PAL["accent"], text_color=PAL["accent"],
            hover_color=("#EFF6FF", "#1E3A5F"),
            font=("SF Pro Text", 13),
            command=self._export_csv,
            state="disabled",
        )
        self._export_btn.pack(side="left", padx=(10, 0))

        # Thanh tiến trình (ẩn mặc định)
        self._progress = ctk.CTkProgressBar(
            ctrl, width=200, height=8, corner_radius=4,
            fg_color=("#E2E8F0", "#1E293B"), progress_color=PAL["accent"],
        )
        self._progress.set(0)
        self._progress.pack(side="left", padx=(20, 0))
        self._progress.pack_forget()  # ẩn ban đầu

        self._status_lbl = ctk.CTkLabel(
            ctrl, text="", font=FONT_SMALL, text_color=PAL["text_muted"],
        )
        self._status_lbl.pack(side="left", padx=10)

        # ── Metric Cards ─────────────────────────────────────────────────────
        metrics_row = ctk.CTkFrame(self, fg_color="transparent")
        metrics_row.pack(fill="x", padx=24, pady=(16, 0))

        self._card_accuracy = MetricCard(
            metrics_row, title="Độ chính xác", value="—",
            accent=PAL["accent"],
        )
        self._card_accuracy.pack(side="left", expand=True, fill="x", padx=(0, 8))

        self._card_fixed = MetricCard(
            metrics_row, title="File đề xuất đổi nhãn", value="—",
            accent=PAL["warning"],
        )
        self._card_fixed.pack(side="left", expand=True, fill="x", padx=(0, 8))

        self._card_total = MetricCard(
            metrics_row, title="Tổng file báo cáo", value="—",
            accent=PAL["success"],
        )
        self._card_total.pack(side="left", expand=True, fill="x")

        # ── Bảng dữ liệu ─────────────────────────────────────────────────────
        ctk.CTkLabel(
            self, text="KẾT QUẢ ĐỀ XUẤT ĐỔI NHÃN",
            font=("SF Pro Text", 10, "bold"), text_color=PAL["text_muted"],
            anchor="w",
        ).pack(fill="x", padx=24, pady=(18, 4))

        self._table = DataTable(self)
        self._table.pack(fill="both", expand=True, padx=24, pady=(0, 20))

    # ── Sự kiện ──────────────────────────────────────────────────────────────

    def _browse_csv(self) -> None:
        path = filedialog.askopenfilename(
            title="Chọn file báo cáo CSV",
            filetypes=[("CSV files", "*.csv"), ("All files", "*.*")],
        )
        if not path:
            return
        self._csv_path = Path(path)
        self._csv_entry.delete(0, "end")
        self._csv_entry.insert(0, str(self._csv_path))

    def _run_analysis(self) -> None:
        if not self._csv_path or not self._csv_path.exists():
            messagebox.showerror("Lỗi", "Vui lòng chọn file classification_report.csv hợp lệ.")
            return
        if not self._app.aliases:
            messagebox.showerror("Lỗi", "Chưa nạp được dữ liệu aliases. Kiểm tra Tab 2.")
            return

        self._run_btn.configure(state="disabled", text="Đang phân tích...")
        self._export_btn.configure(state="disabled")
        self._progress.pack(side="left", padx=(20, 0))
        self._progress.set(0)
        self._set_status("Đang đọc báo cáo...")

        def _worker():
            try:
                rows = load_report(self._csv_path)
                total = len(rows)

                def _progress_cb(ratio: float):
                    self._progress.set(ratio)
                    self._set_status(f"Đang xử lý {int(ratio * total)}/{total}...")

                changes, accuracy, n_fixed = analyse_report(
                    rows, self._app.aliases, _progress_cb
                )
                self._result_rows = changes
                self.after(0, lambda: self._on_done(changes, accuracy, n_fixed, total))
            except Exception as exc:
                self.after(0, lambda: self._on_error(str(exc)))

        threading.Thread(target=_worker, daemon=True).start()

    def _on_done(self, changes: list[dict], accuracy: float, n_fixed: int, total: int) -> None:
        self._card_accuracy.set_value(f"{accuracy:.1f}%")
        self._card_fixed.set_value(str(n_fixed))
        self._card_total.set_value(str(total))
        self._table.load(changes)
        self._progress.set(1.0)
        self._set_status(f"Hoàn tất — {n_fixed} đề xuất đổi nhãn")
        self._run_btn.configure(state="normal", text="▶  Bắt đầu phân tích")
        if changes:
            self._export_btn.configure(state="normal")
        self.after(3000, lambda: self._progress.pack_forget())

    def _on_error(self, msg: str) -> None:
        self._run_btn.configure(state="normal", text="▶  Bắt đầu phân tích")
        self._progress.pack_forget()
        self._set_status("")
        messagebox.showerror("Lỗi phân tích", msg)

    def _set_status(self, text: str) -> None:
        self._status_lbl.configure(text=text)

    def _export_csv(self) -> None:
        if not self._result_rows:
            return
        path = filedialog.asksaveasfilename(
            title="Lưu kết quả",
            defaultextension=".csv",
            filetypes=[("CSV files", "*.csv")],
            initialfile="de_xuat_doi_nhan.csv",
        )
        if not path:
            return
        try:
            with open(path, "w", encoding="utf-8-sig", newline="") as f:
                writer = csv.DictWriter(
                    f, fieldnames=["filename", "old_label", "new_label", "status"]
                )
                writer.writeheader()
                writer.writerows(self._result_rows)
            messagebox.showinfo("Xuất thành công", f"Đã lưu {len(self._result_rows)} dòng vào:\n{path}")
        except Exception as exc:
            messagebox.showerror("Lỗi xuất file", str(exc))


# ═════════════════════════════════════════════════════════════════════════════
# TAB 2: QUẢN LÝ TỪ KHÓA (Aliases)
# ═════════════════════════════════════════════════════════════════════════════

class KeywordTab(ctk.CTkFrame):
    def __init__(self, master, app: "App", **kw):
        super().__init__(master, fg_color="transparent", **kw)
        self._app = app
        self._aliases_path: Path | None = None
        self._build()

    def _build(self) -> None:
        # ── Tiêu đề ──────────────────────────────────────────────────────────
        header = ctk.CTkFrame(self, fg_color="transparent")
        header.pack(fill="x", padx=24, pady=(20, 0))

        ctk.CTkLabel(
            header, text="Quản lý từ khóa (Aliases)",
            font=("SF Pro Display", 20, "bold"),
        ).pack(side="left")

        # ── Chọn file aliases.json ────────────────────────────────────────────
        file_frame = ctk.CTkFrame(self, corner_radius=12)
        file_frame.pack(fill="x", padx=24, pady=(16, 0))

        ctk.CTkLabel(
            file_frame, text="FILE ALIASES.JSON",
            font=("SF Pro Text", 10, "bold"), text_color=PAL["text_muted"],
        ).grid(row=0, column=0, sticky="w", padx=16, pady=(14, 0))

        self._alias_entry = ctk.CTkEntry(
            file_frame, placeholder_text="Chọn file aliases.json...",
            font=FONT_MONO, height=36, corner_radius=8,
        )
        self._alias_entry.grid(row=1, column=0, sticky="ew", padx=16, pady=(4, 14))

        ctk.CTkButton(
            file_frame, text="Browse", width=90, height=36,
            corner_radius=8, fg_color=PAL["accent"], hover_color=PAL["accent_dark"],
            command=self._browse_aliases,
        ).grid(row=1, column=1, padx=(0, 16), pady=(4, 14))

        file_frame.columnconfigure(0, weight=1)

        # ── Dropdown chọn môn ────────────────────────────────────────────────
        picker_row = ctk.CTkFrame(self, fg_color="transparent")
        picker_row.pack(fill="x", padx=24, pady=(14, 0))

        ctk.CTkLabel(
            picker_row, text="MÔN HỌC",
            font=("SF Pro Text", 10, "bold"), text_color=PAL["text_muted"],
        ).pack(anchor="w")

        subject_row = ctk.CTkFrame(picker_row, fg_color="transparent")
        subject_row.pack(fill="x", pady=(4, 0))

        self._subject_var = ctk.StringVar(value="— Chọn môn học —")
        self._subject_combo = ctk.CTkComboBox(
            subject_row,
            variable=self._subject_var,
            values=["— Chọn môn học —"],
            width=260, height=38, corner_radius=8,
            font=FONT_LABEL,
            command=self._on_subject_change,
            state="disabled",
        )
        self._subject_combo.pack(side="left")

        # Nút thêm môn mới
        ctk.CTkButton(
            subject_row, text="+ Thêm môn",
            width=110, height=38, corner_radius=8,
            fg_color="transparent", border_width=1,
            border_color=PAL["accent"], text_color=PAL["accent"],
            hover_color=("#EFF6FF", "#1E3A5F"),
            font=FONT_SMALL,
            command=self._add_subject,
        ).pack(side="left", padx=(10, 0))

        # Nút xóa môn
        ctk.CTkButton(
            subject_row, text="Xóa môn này",
            width=110, height=38, corner_radius=8,
            fg_color="transparent", border_width=1,
            border_color=PAL["danger"], text_color=PAL["danger"],
            hover_color=("#FEF2F2", "#3B1515"),
            font=FONT_SMALL,
            command=self._delete_subject,
        ).pack(side="left", padx=(8, 0))

        # ── Label hướng dẫn ──────────────────────────────────────────────────
        guide_frame = ctk.CTkFrame(self, corner_radius=8, fg_color=("#EFF6FF", "#172554"))
        guide_frame.pack(fill="x", padx=24, pady=(12, 0))

        ctk.CTkLabel(
            guide_frame,
            text="ℹ  Nhập các từ khóa phân cách bằng dấu PHẨY. "
                 "Ví dụ:  toan, toan hoc, mon toan, math, hinh hoc",
            font=FONT_SMALL, text_color=("#1D4ED8", "#93C5FD"),
            anchor="w",
        ).pack(padx=12, pady=8)

        # ── Khung nhập từ khóa ───────────────────────────────────────────────
        ctk.CTkLabel(
            self, text="TỪ KHÓA HIỆN TẠI",
            font=("SF Pro Text", 10, "bold"), text_color=PAL["text_muted"],
            anchor="w",
        ).pack(fill="x", padx=24, pady=(14, 4))

        self._keyword_box = ctk.CTkTextbox(
            self,
            corner_radius=10,
            font=FONT_MONO,
            wrap="word",
            height=200,
            border_width=1,
        )
        self._keyword_box.pack(fill="both", expand=True, padx=24)
        self._keyword_box.insert("end", "Chọn môn học từ danh sách bên trên để chỉnh sửa từ khóa...")
        self._keyword_box.configure(state="disabled")

        # ── Nút lưu ──────────────────────────────────────────────────────────
        save_row = ctk.CTkFrame(self, fg_color="transparent")
        save_row.pack(fill="x", padx=24, pady=(12, 20))

        self._save_btn = ctk.CTkButton(
            save_row, text="💾  Lưu cấu hình",
            height=42, corner_radius=8,
            fg_color=PAL["success"], hover_color="#16A34A",
            font=("SF Pro Text", 13, "bold"),
            command=self._save_aliases,
            state="disabled",
        )
        self._save_btn.pack(side="left")

        self._save_status = ctk.CTkLabel(
            save_row, text="", font=FONT_SMALL, text_color=PAL["text_muted"],
        )
        self._save_status.pack(side="left", padx=14)

    # ── Sự kiện ──────────────────────────────────────────────────────────────

    def _browse_aliases(self) -> None:
        path = filedialog.askopenfilename(
            title="Chọn file aliases.json",
            filetypes=[("JSON files", "*.json"), ("All files", "*.*")],
        )
        if not path:
            return
        self._aliases_path = Path(path)
        self._alias_entry.delete(0, "end")
        self._alias_entry.insert(0, str(self._aliases_path))

        try:
            data = load_aliases(self._aliases_path)
            self._app.aliases = data
            self._app.aliases_path = self._aliases_path
            self._refresh_combo()
            self._subject_combo.configure(state="normal")
            self._save_status.configure(text=f"Đã nạp {len(data)} môn học", text_color=PAL["success"])
        except Exception as exc:
            messagebox.showerror("Lỗi nạp aliases", str(exc))

    def _refresh_combo(self) -> None:
        subjects = list(self._app.aliases.keys())
        self._subject_combo.configure(values=["— Chọn môn học —"] + subjects)
        self._subject_var.set("— Chọn môn học —")

    def _on_subject_change(self, value: str) -> None:
        """Khi chọn môn → nạp danh sách từ khóa vào textbox."""
        if value.startswith("—"):
            self._keyword_box.configure(state="disabled")
            self._save_btn.configure(state="disabled")
            return

        keywords = self._app.aliases.get(value, [])
        self._keyword_box.configure(state="normal")
        self._keyword_box.delete("1.0", "end")
        self._keyword_box.insert("end", ", ".join(keywords))
        self._save_btn.configure(state="normal")
        self._save_status.configure(text="")

    def _save_aliases(self) -> None:
        subject = self._subject_var.get()
        if subject.startswith("—"):
            return

        raw_text = self._keyword_box.get("1.0", "end").strip()

        # Cắt theo dấu phẩy, strip từng từ, bỏ rỗng
        keywords = [kw.strip().lower() for kw in raw_text.split(",") if kw.strip()]

        if not keywords:
            messagebox.showwarning(
                "Dữ liệu không hợp lệ",
                f"Môn '{subject}' cần ít nhất 1 từ khóa.",
            )
            return

        # Cập nhật bộ nhớ
        self._app.aliases[subject] = keywords

        # Ghi về file JSON
        if not self._aliases_path:
            messagebox.showerror("Lỗi", "Không xác định được đường dẫn file aliases.json.")
            return

        try:
            save_aliases(self._aliases_path, self._app.aliases)
            self._save_status.configure(
                text=f"✓ Đã lưu {len(keywords)} từ khóa cho '{subject}'",
                text_color=PAL["success"],
            )
            # Làm sạch trạng thái sau 4 giây
            self.after(4000, lambda: self._save_status.configure(text="", text_color=PAL["text_muted"]))
        except Exception as exc:
            messagebox.showerror("Lỗi ghi file", str(exc))

    def _add_subject(self) -> None:
        dialog = ctk.CTkInputDialog(
            text="Nhập tên môn học mới (ví dụ: Cong_nghe):",
            title="Thêm môn học",
        )
        name = dialog.get_input()
        if not name or not name.strip():
            return
        name = name.strip()
        if name in self._app.aliases:
            messagebox.showwarning("Trùng tên", f"Môn '{name}' đã tồn tại.")
            return
        self._app.aliases[name] = []
        self._refresh_combo()
        self._subject_var.set(name)
        self._on_subject_change(name)

    def _delete_subject(self) -> None:
        subject = self._subject_var.get()
        if subject.startswith("—"):
            return
        confirm = messagebox.askyesno(
            "Xác nhận xóa",
            f"Xóa môn '{subject}' và toàn bộ từ khóa của môn này?\nKhông thể hoàn tác.",
        )
        if not confirm:
            return
        self._app.aliases.pop(subject, None)
        # Ghi ngay xuống file nếu đang mở
        if self._aliases_path:
            try:
                save_aliases(self._aliases_path, self._app.aliases)
            except Exception:
                pass
        self._refresh_combo()
        self._keyword_box.configure(state="disabled")
        self._keyword_box.delete("1.0", "end")
        self._keyword_box.insert("end", "Chọn môn học từ danh sách bên trên để chỉnh sửa từ khóa...")
        self._save_btn.configure(state="disabled")
        self._save_status.configure(
            text=f"Đã xóa môn '{subject}'", text_color=PAL["warning"]
        )


# ═════════════════════════════════════════════════════════════════════════════
# APP ROOT
# ═════════════════════════════════════════════════════════════════════════════

class App(ctk.CTk):
    """Cửa sổ chính của ứng dụng."""

    def __init__(self):
        super().__init__()

        # Trạng thái dùng chung giữa các tab
        self.aliases: dict[str, list[str]] = {}
        self.aliases_path: Path | None = None

        self.title("Phân loại Tài liệu THPT — Phân tích & Tối ưu")
        self.geometry("980x720")
        self.minsize(820, 580)

        self._build()

    def _build(self) -> None:
        # ── Sidebar / Navigation ──────────────────────────────────────────────
        sidebar = ctk.CTkFrame(self, width=200, corner_radius=0)
        sidebar.pack(side="left", fill="y")
        sidebar.pack_propagate(False)

        # Logo / tên app
        logo_frame = ctk.CTkFrame(sidebar, fg_color="transparent")
        logo_frame.pack(fill="x", pady=(24, 0), padx=16)

        ctk.CTkLabel(
            logo_frame, text="DocSort",
            font=("SF Pro Display", 18, "bold"),
            text_color=PAL["accent"],
        ).pack(anchor="w")
        ctk.CTkLabel(
            logo_frame, text="Phân loại tài liệu THPT",
            font=FONT_SMALL, text_color=PAL["text_muted"],
        ).pack(anchor="w")

        ctk.CTkFrame(sidebar, height=1, fg_color=("#E2E8F0", "#1E293B")).pack(
            fill="x", padx=16, pady=20
        )

        # Navigation buttons
        self._tab_btns: list[ctk.CTkButton] = []
        self._tab_frames: list[ctk.CTkFrame] = []

        nav_items = [
            ("📊  Phân tích & Tối ưu", self._make_analysis_tab),
            ("🔑  Quản lý từ khóa",    self._make_keyword_tab),
        ]

        for idx, (label, factory) in enumerate(nav_items):
            btn = ctk.CTkButton(
                sidebar, text=label, anchor="w",
                height=40, corner_radius=8,
                fg_color="transparent",
                text_color=("#1E293B", "#E2E8F0"),
                hover_color=("#EFF6FF", "#1E3A5F"),
                font=FONT_LABEL,
                command=lambda i=idx: self._switch_tab(i),
            )
            btn.pack(fill="x", padx=10, pady=2)
            self._tab_btns.append(btn)

        # Spacer + theme toggle
        ctk.CTkFrame(sidebar, fg_color="transparent").pack(fill="y", expand=True)

        theme_row = ctk.CTkFrame(sidebar, fg_color="transparent")
        theme_row.pack(fill="x", padx=16, pady=(0, 20))

        ctk.CTkLabel(theme_row, text="Chủ đề", font=FONT_SMALL).pack(side="left")
        ctk.CTkSwitch(
            theme_row, text="", width=44,
            command=self._toggle_theme,
        ).pack(side="right")

        # ── Content area ─────────────────────────────────────────────────────
        self._content = ctk.CTkFrame(self, corner_radius=0, fg_color=("gray95", "gray10"))
        self._content.pack(side="left", fill="both", expand=True)

        # Khởi tạo các tab (lazy: chỉ tạo frame khi cần)
        self._analysis_tab: AnalysisTab | None = None
        self._keyword_tab:  KeywordTab  | None = None
        self._current_tab  = -1

        # Hiển thị tab đầu tiên
        self._switch_tab(0)

    # ── Factory methods để tạo tab lần đầu ──────────────────────────────────

    def _make_analysis_tab(self) -> AnalysisTab:
        if self._analysis_tab is None:
            self._analysis_tab = AnalysisTab(self._content, app=self)
        return self._analysis_tab

    def _make_keyword_tab(self) -> KeywordTab:
        if self._keyword_tab is None:
            self._keyword_tab = KeywordTab(self._content, app=self)
        return self._keyword_tab

    _tab_factories = [_make_analysis_tab, _make_keyword_tab]

    def _switch_tab(self, idx: int) -> None:
        if idx == self._current_tab:
            return

        # Ẩn tab cũ
        for frame in self._content.winfo_children():
            frame.pack_forget()

        # Tạo / hiện tab mới
        tab = self._tab_factories[idx](self)
        tab.pack(fill="both", expand=True)
        self._current_tab = idx

        # Cập nhật style nút nav
        for i, btn in enumerate(self._tab_btns):
            if i == idx:
                btn.configure(
                    fg_color=(PAL["accent"], PAL["accent_dark"]),
                    text_color="white",
                )
            else:
                btn.configure(
                    fg_color="transparent",
                    text_color=("#1E293B", "#E2E8F0"),
                )

    def _toggle_theme(self) -> None:
        current = ctk.get_appearance_mode()
        ctk.set_appearance_mode("Light" if current == "Dark" else "Dark")


# ═════════════════════════════════════════════════════════════════════════════
# ENTRY POINT
# ═════════════════════════════════════════════════════════════════════════════

if __name__ == "__main__":
    app = App()
    app.mainloop()